...
2. Actions → **Ops - Supabase Bootstrap** → Run workflow（**full_reset=false** で `0002+` のみ。初回のみ full_reset=true）
3. main push で Vercel deploy（コード変更時）

> **接続文字列（`SUPABASE_DB_URL`）は不要。**  
> 2026-07 時点: DDL 適用は [Management API](https://supabase.com/docs/guides/integrations/supabase-for-platforms) が標準。  
> 人間が psql する場合のみ Dashboard 上部 **Connect** → Connection string → URI（[公式](https://supabase.com/docs/guides/database/connecting-to-postgres)）。
...
- `0002_supporter_contact_visibility.sql` 未適用の DB では連絡先が一覧に出ない（Bootstrap **full_reset=false** で適用）
