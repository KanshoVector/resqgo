...
1. GitHub Repository secrets に追加:
   - `SUPABASE_URL` / `SUPABASE_ANON_KEY`（Vercel の `NEXT_PUBLIC_*` と同値）
   - `SUPABASE_DB_URL`（Dashboard → Database → Connection string URI）
2. Actions → **Ops - Supabase Bootstrap** → Run workflow  
   → スキーマ適用 + スモークテストまで自動
3. Vercel へ main push（アプリ deploy）

以降の DB 変更も同 workflow を再実行（§0 RESET でデータ消去あり）。
